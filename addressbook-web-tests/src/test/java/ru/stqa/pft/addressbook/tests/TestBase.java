  ...
  public void verifyContactsListInUI() {
    if(Boolean.getBoolean("verifyUI")) {
      ...
      assertThat(uiContacts, equalTo(dbContacts.stream().map((g) ->
              new ContactData().withId(g.getId()).withFirstname(g.getFirstname())
                      .withLastname(g.getLastname()).withAddress(g.getAddress())
                      .withHomeNumber(g.getHomeNumber()).withMobileNumber(g.getMobileNumber())
                      .withWorkNumber(g.getWorkNumber()).withEmail(g.getEmail())
